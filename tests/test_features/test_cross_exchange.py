...
class TestCoinbasePremiumComputer:
    ...
    def test_warmup_returns_nan(self) -> None:
        ...
        result = c.compute(10, ts, opens, highs, lows, closes, vols, coinbase_closes=coinbase)
    ...
    def test_golden_zero_premium(self) -> None:
        ...
        result = c.compute(100, ts, opens, highs, lows, closes, vols, coinbase_closes=coinbase)
    ...
    def test_no_look_ahead_bias(self) -> None:
        ...
        idx = 150
        full = c.compute(idx, ts, opens, highs, lows, closes, vols, coinbase_closes=coinbase)
        trunc = c.compute(
            idx,
            ts[: idx + 1],
            opens[: idx + 1],
        ...
        assert full["coinbase_premium_zscore"] == pytest.approx(
            trunc["coinbase_premium_zscore"], abs=1e-8
        )
...
class TestETHOrderFlowComputer:
    ...
    def test_warmup_returns_nan(self) -> None:
        ...
        result = c.compute(
            ...
            closes,
            vols,
            eth_trade_sizes=eth_sizes,
            eth_trade_sides=eth_sides,
        )
        assert math.isnan(result["eth_net_taker"])
    ...
    def test_no_look_ahead_bias(self) -> None:
        ...
        assert full["eth_net_taker_zscore"] == pytest.approx(
            trunc["eth_net_taker_zscore"], abs=1e-8
        )
...
class TestLongShortRatioComputer:
    ...
    def test_warmup_returns_nan(self) -> None:
        ...
        ts, opens, highs, lows, closes, vols = _make_prices(n)
        ls = np.ones(n) * 1.5
        result = c.compute(50, ts, opens, highs, lows, closes, vols, long_short_ratio=ls)
        assert math.isnan(result["ls_ratio"])
    ...
    def test_golden_ratio_passed_through(self) -> None:
        ...
        ts, opens, highs, lows, closes, vols = _make_prices(n)
        ls = np.ones(n) * 1.5
        result = c.compute(150, ts, opens, highs, lows, closes, vols, long_short_ratio=ls)
    ...
    def test_no_look_ahead_bias(self) -> None:
        ...
        idx = 150
        full = c.compute(idx, ts, opens, highs, lows, closes, vols, long_short_ratio=ls)
        trunc = c.compute(
            idx,
            ts[: idx + 1],
            opens[: idx + 1],
...
class TestCrossExchangeOFIComputer:
    ...
    def test_golden_zero_divergence(self) -> None:
        ...
        result = c.compute(
            50, ts, opens, highs, lows, closes, vols, binance_ofi=ofi, coinbase_ofi=ofi
        )
    ...
    def test_no_look_ahead_bias(self) -> None:
        ...
        assert full["xex_ofi_divergence_ma"] == pytest.approx(
            trunc["xex_ofi_divergence_ma"], abs=1e-10
        )
